  ...
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"><i class="fa fa-list" aria-hidden="true"></i> ADD DATA NELAYAN</h3>
        <div class="nav-back">
      ...
      <div class="tile-body">
        <form id="form-add" method="post" role="form" class="was-validated" action="<?php echo base_url();?>nelayan/save">
          <div class="row">
            <div class="col-md-4">
              <div class="form-group">
                <label class="control-label">Pas Photo <i>( jpg|jpeg|png )</i></label>
            ...
            <div class="col-md-4">
              <div class="form-group">
                ...
                <input type="text" class="form-control" name="nama_nelayan" id="nama_nelayan" required autofocus>
                <div class="valid-feedback"><i>*valid</i> </div>
                <div class="invalid-feedback"><i>*required</i> </div>
              </div>
            </div>
            ...
            <div class="col-md-4">
              <div class="form-group">
                <label class="control-label">Kepemilikan Perahu</label>
                <div class="input-group mb-3">
                  <select class="form-control boot-select" name="kepemilikkan_perahu" id="kepemilikkan_perahu"  required>
                    <option value=""></option>
            ...
            <div class="col-md-4">
              <div class="form-group">
                <label class="control-label">Metode Tangkap</label>
                <div class="input-group mb-3">
                  <select class="form-control boot-select" name="id_metode_tangkap" id="id_metode_tangkap"  required>
                    <option value=""></option>
                    <?php
                    foreach ( $metode as $k ) {
                      echo '<option value="' . encrypt_url( $k->id_metode_tangkap ) . '">' . $k->metode_tangkap . '</option>';
                    }
                    ?>
                  </select>
                  <div class="valid-feedback"><i>*valid</i> </div>
          ...
          <table id="table" class="table table-bordered">
            <thead>
              <tr>
                ...
                <th>NAMA NELAYAN</th>
                <th>NAMA PERAHU</th>
                <th>KEPEMILIKKAN PERAHU</th>
                <th>NAMA KELOMPOK</th>
                <th>METODE TANGKAP</th>
              </tr>
            </thead>
            <tbody>
            </tbody>
...
<script type="text/javascript">
	...
	var table = $('#table').DataTable({
	 ...
	 "processing": true,
		...
		"ajax": {
			"url": "<?php echo site_url('nelayan/ajax_list')?>",
			"type": "POST"
		},
		"columnDefs": [{
	...
	function reset_form() {
		...
		$('#form-add').attr('action', '<?php echo base_url();?>nelayan/save');
		$('#inputan').addClass('sembunyi');
		$('#isi-datane').removeClass('sembunyi');
	...
	function edit_data(id) {
		$('.loading').fadeIn('fast');
		$('#form-add')[0].reset();
		$('#form-add').attr('action', "<?php echo base_url('nelayan/update?q=');?>" + id);
		$.ajax({
			url: "<?php echo site_url('nelayan/edit')?>",
			type: "POST",
			data: {
				q: id
			},
			...
			success: function(data) {
				$('.loading').fadeOut('fast');
				$('#nama_nelayan').val(data.nama_nelayan);
				$('#id_kelompok').val(data.id_kelompok).trigger('change');
				$('#id_perahu').val(data.id_perahu).trigger('change');
				$('#id_metode_tangkap').val(data.id_metode_tangkap).trigger('change');
				$('#kepemilikkan_perahu').val(data.kepemilikkan_perahu).trigger('change');
	...
	function delete_data(x) {
		Swal.fire({
			...
			text: "Apakah anda yakin akan menghapus data nelayan ini?",
			type: "warning",
			...
			if (result.value) {
				$('.loading').fadeIn('fast');
				$.ajax({
					url: "<?php echo site_url('nelayan/delete')?>",
					type: "POST",
					data: {
						q: x
					},
