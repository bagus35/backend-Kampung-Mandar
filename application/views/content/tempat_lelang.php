  ...
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"><i class="fa fa-list" aria-hidden="true"></i> ADD TEMPAT LELANG</h3>
        <div class="nav-back">
      ...
      <div class="tile-body">
        <form id="form-add" method="post" role="form" class="was-validated" action="<?php echo base_url();?>tempat_lelang/save">
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                <label class="col-form-label">Nama Tempat</label>
                <input type="text" class="form-control" name="tempat_lelang" id="tempat_lelang" required autofocus>
...
<div id="isi-datane" class="row">
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"><i class="fa fa-list" aria-hidden="true"></i> TEMPAT LELANG</h3>
        <div class="nav-back">
          <div class="btn-group" role="group" aria-label="Basic example">
...
<script type="text/javascript">
	...
	function reset_form() {
		...
		$('#form-add').attr('action', '<?php echo base_url();?>tempat_lelang/save');
		$('#inputan').addClass('sembunyi');
		$('#isi-datane').removeClass('sembunyi');
		$('#nama_ikan').focus();
	}
	function edit_data(id) {
		$('.loading').fadeIn('fast');
		$('#form-add')[0].reset();
		$('#form-add').attr('action', "<?php echo base_url('tempat_lelang/update?q=');?>" + id);
		$.ajax({
			url: "<?php echo site_url('tempat_lelang/edit')?>",
			type: "POST",
			data: {
				q: id
			},
			...
			success: function(data) {
				$('.loading').fadeOut('fast');
				$('#tempat_lelang').val(data.tempat_lelang);
	...
	function delete_data(x) {
		Swal.fire({
			title: "DELETE TEMPAT LELANG. HATI-HATI!",
			text: "Sebelum anda menghapus data Tempat Lelang ini. Pastikan tidak ada data yang terkait dengan tempat lelang ini. Kesalahan penghapusan menyebabkan terhapusnya data yang terkait dengan tempat lelang ini?",
			type: "warning",
			...
			if (result.value) {
				$('.loading').fadeIn('fast');
				$.ajax({
					url: "<?php echo site_url('tempat_lelang/delete')?>",
					type: "POST",
					data: {
						q: x
					},
	...
	var table = $('#table').DataTable({
	 ...
	 "processing": true,
		...
		"ajax": {
			"url": "<?php echo site_url('tempat_lelang/ajax_list')?>",
			"type": "POST"
		},
		"columnDefs": [{
