  ...
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"><i class="fa fa-list" aria-hidden="true"></i> ADD Metode Tangkap</h3>
        <div class="nav-back">
      ...
      <div class="tile-body">
        <form id="form-add" method="post" role="form" class="was-validated" action="<?php echo base_url();?>metode_tangkap/save">
          <div class="row">

            <div class="col-md-12">
              <div class="form-group">
                <label class="col-form-label">Nama Metode Tangkap</label>
                <input type="text" class="form-control" name="metode_tangkap" id="metode_tangkap" required>
...
<div id="isi-datane" class="row">
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"> <i class="fa fa-list" aria-hidden="true"></i> METODE TANGKAP</h3>
        <div class="nav-back">
          <div class="btn-group" role="group" aria-label="Basic example">
...
<script type="text/javascript">
	...
	function reset_form() {
		...
		$('#form-add').attr('action', '<?php echo base_url();?>metode_tangkap/save');
		$('#inputan').addClass('sembunyi');
		$('#isi-datane').removeClass('sembunyi');
		$('#metode_tangkap').focus();
	}
	function edit_data(id) {
		$('.loading').fadeIn('fast');
		$('#form-add')[0].reset();
		$('#form-add').attr('action', "<?php echo base_url('metode_tangkap/update?q=');?>" + id);
		$.ajax({
			url: "<?php echo site_url('metode_tangkap/edit')?>",
			type: "POST",
			data: {
				q: id
			},
			...
			success: function(data) {
				$('.loading').fadeOut('fast');
				$('#metode_tangkap').val(data.metode_tangkap);
	...
	function delete_data(x) {
		Swal.fire({
			title: "DELETE METODE TANGKAP. HATI-HATI!",
			text: "Sebelum anda menghapus metode tangkap ini. Pastikan tidak ada data yang terkait dengan metode tangkap ini. Kesalahan penghapusan menyebabkan terhapusnya data yang terkait dengan metode tangkap ini?",
			type: "warning",
			...
			if (result.value) {
				$('.loading').fadeIn('fast');
				$.ajax({
					url: "<?php echo site_url('metode_tangkap/delete')?>",
					type: "POST",
					data: {
						q: x
					},
	...
	var table = $('#table').DataTable({
		...
		"ajax": {
			"url": "<?php echo site_url('metode_tangkap/ajax_list')?>",
			"type": "POST"
		},
		"columnDefs": [{
