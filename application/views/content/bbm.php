  ...
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"><i class="fa fa-list" aria-hidden="true"></i> ADD KONSUMSI BBM</h3>
        <div class="nav-back">
      ...
      <div class="tile-body">
        <form id="form-add" method="post" role="form" class="was-validated" action="<?php echo base_url();?>konsumsi_bbm/save">
          <div class="row">
            <div class="col-md-4">
              ...
              <div class="form-group">
                ...
                <div class="input-group mb-3">
                  <select class="form-control boot-select" name="id_perahu" id="id_perahu"  required>
                    ...
                    foreach ( $perahu as $k ) {
                      echo '<option value="' . 
					  encrypt_url( $k->id_perahu ) . '">' . 
					  $k->nama_perahu . '</option>';
                    }
                    ?>
                  </select>
                  <div class="valid-feedback"><i>*valid</i> </div>
            ...
            <div class="col-md-4">
              <div class="form-group">
                ...
                <div class="input-group mb-3">
                  <select class="form-control boot-select" name="jenis" id="jenis"  required>
            ...
            <div class="col-md-4">
              <div class="form-group">
                <label class="control-label">Harga Per Liter</label>
                <input type="text" class="form-control ribuan" name="harga_per_liter" id="harga_per_liter" required>
                <div class="valid-feedback"><i>*valid</i> </div>
                <div class="invalid-feedback"><i>*required</i> </div>
              </div>
            </div>
            <div class="col-md-4">
              <div class="form-group">
                <label class="control-label">Jumlah Konsumsi (liter)</label>
                <input type="text" class="form-control ribuan" name="jumlah" id="jumlah" required>
                <div class="valid-feedback"><i>*valid</i> </div>
                <div class="invalid-feedback"><i>*required</i> </div>
              </div>
            </div>
...
<div id="isi-datane" class="row">
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"><i class="fa fa-list" aria-hidden="true"></i> DATA KONSUMSI BBM</h3>
        <div class="nav-back">
          <div class="btn-group" role="group" aria-label="Basic example">
...
<script type="text/javascript">
	$('#bbm').addClass('active');
	$('#produksi').addClass('radius-bawah');
	...
	var table = $('#table').DataTable({
	 ...
	 "processing": true,
		...
		"ajax": {
			"url": "<?php echo site_url('konsumsi_bbm/ajax_list')?>",
			"type": "POST"
		},
		"columnDefs": [{
	...
	function reset_form() {
		...
		$('#form-add').attr('action', '<?php echo base_url();?>konsumsi_bbm/save');
		$('#inputan').addClass('sembunyi');
		$('#isi-datane').removeClass('sembunyi');
		$('.boot-select').val("").trigger('change');
		$('.boot-select-no').val("").trigger('change');
	...
	function edit_data(id) {
		...
		$('.boot-select').val("").trigger('change');
		$('.boot-select-no').val("").trigger('change');
		$('#form-add').attr('action', "<?php echo base_url('konsumsi_bbm/update?q=');?>" + id);
		$.ajax({
			url: "<?php echo site_url('konsumsi_bbm/edit')?>",
			type: "POST",
			data: {
				q: id
			},
			...
			success: function(data) {
				...
				$('#harga_per_liter').val(data.harga_per_liter);
	...
	function delete_data(x) {
		Swal.fire({
			...
			if (result.value) {
				$('.loading').fadeIn('fast');
				$.ajax({
					url: "<?php echo site_url('konsumsi_bbm/delete')?>",
					type: "POST",
					data: {
						q: x
					},
