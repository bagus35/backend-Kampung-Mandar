...
<div id="inputan"class="row sembunyi">
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        ...
        <div class="nav-back">
          ...
          aria-label="Basic example">
            ...
            id="btn-close"><i class="fa fa-times" 
      ...
      <div class="tile-body">
        <form id="form-add" 
        method="post" role="form" class="was-validated" 
        action="<?php echo base_url();?>pembukuan/save">
          <div class="row">
            <div class="col-md-4">
              <div class="form-group">
                <label class="control-label">Tanggal</label>
                <input type="text" class="form-control tgl" 
                placeholder="__/__/____" name="tanggal" 
                id="tanggal" value="<?php echo date('d/m/Y');?>" required autofocus>
            ...
            <div class="col-md-4">
              <div class="form-group">
                ...
                <input type="text" class="form-control ribuan"
            ...
            <div class="col-md-4">
              <div class="form-group">
                <label class="control-label">Jumlah Pengeluaran (liter)</label>
                <input type="text" class="form-control ribuan" 
                name="jumlah_pengeluaran" id="jumlah_pengeluaran" required>
                <div class="valid-feedback"><i>*valid</i> </div>
                <div class="invalid-feedback"><i>*required</i> </div>
              </div>
            </div>
...
<div id="isi-datane" class="row">
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"><i class="fa fa-list" aria-hidden="true"></i> DATA PEMBUKUAN KELOMPOK</h3>
        <div class="nav-back">
...
<script type="text/javascript">
	...
	var table = $('#table').DataTable({
	 ...
	 "processing": true,
		...
		"ajax": {
			"url": "<?php echo site_url('pembukuan_kelompok/ajax_list')?>",
			"type": "POST"
		},
		"columnDefs": [{
	...
	function reset_form() {
		...
		$('#form-add').attr('action', '<?php echo base_url();?>pembukuan/save');
		$('#inputan').addClass('sembunyi');
		$('#isi-datane').removeClass('sembunyi');
		$('.boot-select').val("").trigger('change');
		$('.boot-select-no').val("").trigger('change');
	...
	function edit_data(id) {
		...
		$('.boot-select').val("").trigger('change');
		$('.boot-select-no').val("").trigger('change');
		$('#form-add').attr('action', "<?php echo base_url('pembukuan/update?q=');?>" + id);
		$.ajax({
			url: "<?php echo site_url('pembukuan/edit')?>",
			type: "POST",
			data: {
				q: id
			},
	...
	function delete_data(x) {
		Swal.fire({
			...
			if (result.value) {
				$('.loading').fadeIn('fast');
				$.ajax({
					url: "<?php echo site_url('pembukuan/delete')?>",
					type: "POST",
					data: {
						q: x
					},
