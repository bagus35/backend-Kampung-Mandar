  ...
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"><i class="fa fa-list" aria-hidden="true"></i> ADD KELOMPOK NELAYAN</h3>
        <div class="nav-back">
      ...
      <div class="tile-body">
        <form id="form-add" method="post" role="form" class="was-validated" action="<?php echo base_url();?>kelompok_nelayan/save">
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label class="col-form-label">Nama Kelompok</label>
                <input type="text" class="form-control" name="nama_kelompok" id="nama_kelompok" required autofocus>
			  ...
			  <div class="col-md-6">
              <div class="form-group">
                <label class="col-form-label">Nama Ketua</label>
...
<script type="text/javascript">
	...
	function reset_form() {
		...
		$('#form-add').attr('action', '<?php echo base_url();?>kelompok_nelayan/save');
		$('#inputan').addClass('sembunyi');
		$('#isi-datane').removeClass('sembunyi');
		$('#nama_kelompok').focus();
	}
	function edit_data(id) {
		$('.loading').fadeIn('fast');
		$('#form-add')[0].reset();
		$('#form-add').attr('action', "<?php echo base_url('kelompok_nelayan/update?q=');?>" + id);
		$.ajax({
			url: "<?php echo site_url('kelompok_nelayan/edit')?>",
			type: "POST",
			data: {
				q: id
			},
	...
	function delete_data(x) {
		Swal.fire({
			title: "DELETE KELOMPOK NELAYAN. HATI-HATI!",
			text: "Sebelum anda menghapus kelompbok ini. Pastikan tidak ada data yang terkait dengan kelompok ini. Kesalahan penghapusan menyebabkan terhapusnya data yang terkait dengan kelompok ini?",
			type: "warning",
			...
			if (result.value) {
				$('.loading').fadeIn('fast');
				$.ajax({
					url: "<?php echo site_url('kelompok_nelayan/delete')?>",
					type: "POST",
					data: {
						q: x
					},
	...
	var table = $('#table').DataTable({
	 ...
	 "processing": true,
		...
		"ajax": {
			"url": "<?php echo site_url('kelompok_nelayan/ajax_list')?>",
			"type": "POST"
		},
		"columnDefs": [{
