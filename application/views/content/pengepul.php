  ...
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"><i class="fa fa-list" aria-hidden="true"></i> TAMBAH DATA PENGEPUL</h3>
        <div class="nav-back">
          <div class="btn-group" role="group" aria-label="Basic example">
            <button class="btn btn-sm btn-danger" id="btn-close"><i class="fa fa-times" aria-hidden="true">
			</i> Close</button>
          </div>
        </div>
      </div>
      <div class="tile-body">
        <form id="form-add" method="post" role="form" class="was-validated" action="<?php echo base_url();?>pengepul/save">
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label class="col-form-label">Nama Pengepul</label>
...
<div id="isi-datane" class="row">
  <div class="col-md-12">
    <div class="tile">
      <div class="title-page">
        <h3 class="tile-title"><i class="fa fa-list" aria-hidden="true"></i> TAMBAH NAMA PENGEPUL</h3>
        <div class="nav-back">
          <div class="btn-group" role="group" aria-label="Basic example">
...
<script type="text/javascript">
	...
	function reset_form() {
		...
		$('#form-add').attr('action', '<?php echo base_url();?>pengepul/save');
		$('#inputan').addClass('sembunyi');
		$('#isi-datane').removeClass('sembunyi');
		$('#nama_pembeli').focus();
	}
	function edit_data(id) {
		$('.loading').fadeIn('fast');
		$('#form-add')[0].reset();
		$('#form-add').attr('action', "<?php echo base_url('pengepul/update?q=');?>" + id);
		$.ajax({
			url: "<?php echo site_url('pengepul/edit')?>",
			type: "POST",
			data: {
				q: id
			},
	...
	function delete_data(x) {
		Swal.fire({
			title: "DELETE NAMA PENGEPUL. HATI-HATI!",
			text: "Sebelum anda menghapus data pengepul ini. Pastikan tidak ada data yang terkait dengan pengepul ini. Kesalahan penghapusan menyebabkan terhapusnya data yang terkait dengan pengepul ini?",
			type: "warning",
			...
			if (result.value) {
				$('.loading').fadeIn('fast');
				$.ajax({
					url: "<?php echo site_url('pengepul/delete')?>",
					type: "POST",
					data: {
						q: x
					},
	...
	var table = $('#table').DataTable({
	 ...
	 "processing": true,
		...
		"ajax": {
			"url": "<?php echo site_url('pengepul/ajax_list')?>",
			"type": "POST"
		},
		"columnDefs": [{
